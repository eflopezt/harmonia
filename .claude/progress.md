...
### Fase 4.4 — Migración IA Multi-Provider (decisión 2026-03-05)
- [ ] Expandir `ConfiguracionSistema`: `ia_provider` → GEMINI|DEEPSEEK|OPENAI|OLLAMA|NINGUNO, nuevo campo `ia_ocr_provider` (GEMINI|NINGUNO), `ia_api_key` (encrypted), migración tareo.0013
- [ ] Reescribir `asistencia/services/ai_service.py`: despacho por provider (Gemini SDK / OpenAI-compatible DeepSeek / Ollama), sin cambiar interfaz pública
- [ ] RAG con pgvector + LlamaIndex: embeddings text-embedding-3-small, vector store en PostgreSQL, ingesta leyes peruanas y criterios internos
- [ ] PDF Splitter IA: sustituir Ollama Llava → Gemini Files API (nativo PDF), mantener preview pdf.js + drag&drop existente
- [ ] OCR PDFs escaneados: Gemini 2.5 Flash via Files API (upload → 1M contexto → extraer texto/tablas)
- [ ] Extracción tablas: pdfplumber complemento (agregar a requirements.txt)
- [ ] Tab IA en configuracion.html: mostrar/ocultar campos según provider seleccionado, probar conexión Gemini/DeepSeek
- [ ] Añadir `pdfplumber` a requirements.txt
...
### Módulos
- [ ] **Nóminas — Bug**: `NoReverseMatch 'nominas_ir5ta_detalle'` en `registro_detalle.html` línea 391 — URL no registrada en `nominas/urls.py`. Requiere crear vista + URL para detalle cálculo IR 5ta categoría por registro.
- [ ] **Nóminas — Boleta de pago**: Rediseño completo de la boleta PDF generada por el sistema. Comparar contra modelos S10 y BUK:
  - Faltan: aportes del empleador (EsSalud 9%), aporte AFP empleado desglosado (aporte+comisión+seguro), IR 5ta retención
  - Faltan: encabezado con logo empresa, RUC, dirección, período
  - Faltan: datos empleado completos (cargo, área, banco, cuenta, AFP/ONP, CUSPP, fecha ingreso)
  - Faltan: sección remuneraciones / descuentos / aportes empresa en columnas separadas
  - Faltan: totales: bruto, descuentos, neto a pagar
  - Falta: firma empresa + sello (configurable desde Identidad Visual)
  - Referencias: boletas S10 y BUK (adjuntadas por usuario: BPAG-08158055/08851995/03689683-2026-MES-02.pdf)
...
- 2026-03-05 (cont.): DECISIÓN ARQUITECTURAL IA — Migración Ollama → Cloud providers. Stack: Gemini 2.5 Flash (OCR/PDFs nativos, 1M ctx) + DeepSeek-V3 via Azure/Groq (RAG queries baratas) + pgvector (PostgreSQL existente) + LlamaIndex + pdfplumber. Privacidad LPDP deprioritizada año 1 (datos no sensibles). MEMORY.md + progress.md actualizados. Fase 4.4 añadida como pendiente. Pendientes anotados: Bug nominas_ir5ta_detalle URL, Boleta PDF rediseño (faltan aportes empleador/AFP/IR5ta/logo/3 columnas), Block 20.3 Plan Plantilla (presupuesto tab, inline edit, import/export Excel).
